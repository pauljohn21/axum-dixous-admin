use axum::extract::{Path, Query};
use axum::response::IntoResponse;
use axum::Json;
use axum::routing::{get, post};
use axum::Router;
use model::dao::sys_dictionary_details;
use model::dto::page_dto::PageRequest;
use model::dto::page_dto::PageResponse;
use model::dto::sys_dictionary_detail_dto::{SysDictionaryDetailInsertDTO, SysDictionaryDetailUpdateDTO};
use service::sys_dictionary_detail_service::SysDictionaryDetailService;
use utils::prelude::{AppError, R};

#[utoipa::path(
    post,
    path = "/api/dictionaryDetail",
    request_body = SysDictionaryDetailInsertDTO,
    responses((status = 200, description = "成功", body = R<sys_dictionary_details::Model>)),
    tag = "字典详情"
)]
pub async fn create(Json(data): Json<SysDictionaryDetailInsertDTO>) -> Result<impl IntoResponse, AppError> {
    let detail = SysDictionaryDetailService::insert(data).await.map_err(AppError::Anyhow)?;
    Ok(R::ok(detail))
}

#[utoipa::path(
    get,
    path = "/api/dictionaryDetail/list",
    params(("page" = Option<u64>, Query, description = "页码"), ("page_size" = Option<u64>, Query, description = "每页条数")),
    responses((status = 200, description = "成功", body = R<PageResponse<sys_dictionary_details::Model>>)),
    tag = "字典详情"
)]
pub async fn list(Query(query): Query<PageRequest>) -> Result<impl IntoResponse, AppError> {
    let result = SysDictionaryDetailService::list(query).await.map_err(AppError::Anyhow)?;
    Ok(R::ok(result))
}

#[utoipa::path(
    get,
    path = "/api/dictionaryDetail/{id}",
    params(("id" = u64, Path, description = "字典详情ID")),
    responses((status = 200, description = "成功", body = R<sys_dictionary_details::Model>)),
    tag = "字典详情"
)]
pub async fn get_by_id(Path(id): Path<u64>) -> Result<impl IntoResponse, AppError> {
    let detail = SysDictionaryDetailService::get_by_id(id).await.map_err(|e| AppError::NotFoundError(e.to_string()))?;
    Ok(R::ok(detail))
}

#[utoipa::path(
    put,
    path = "/api/dictionaryDetail/{id}",
    params(("id" = u64, Path, description = "字典详情ID")),
    request_body = SysDictionaryDetailUpdateDTO,
    responses((status = 200, description = "成功", body = R<sys_dictionary_details::Model>)),
    tag = "字典详情"
)]
pub async fn update(Path(id): Path<u64>, Json(data): Json<SysDictionaryDetailUpdateDTO>) -> Result<impl IntoResponse, AppError> {
    let detail = SysDictionaryDetailService::update(id, data).await.map_err(AppError::Anyhow)?;
    Ok(R::ok(detail))
}

#[utoipa::path(
    delete,
    path = "/api/dictionaryDetail/{id}",
    params(("id" = u64, Path, description = "字典详情ID")),
    responses((status = 200, description = "成功", body = R<serde_json::Value>)),
    tag = "字典详情"
)]
pub async fn delete_detail(Path(id): Path<u64>) -> Result<impl IntoResponse, AppError> {
    SysDictionaryDetailService::delete(id).await.map_err(AppError::Anyhow)?;
    Ok(R::ok(()))
}

pub fn routes() -> Router {
    Router::new()
        .route("/api/dictionaryDetail", post(create))
        .route("/api/dictionaryDetail/list", get(list))
        .route("/api/dictionaryDetail/{id}", get(get_by_id).put(update).delete(delete_detail))
}
