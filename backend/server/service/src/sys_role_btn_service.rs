...
impl SysRoleBtnService {
    pub async fn insert(db: &DatabaseConnection, data: SysRoleBtnInsertDTO) -> Result<sys_role_btns::Model, ServiceError> {
        ...
        Self::get_by_composite_id(db, data.role_id, data.sys_menu_id, data.sys_base_menu_btn_id).await
    }

    pub async fn list(db: &DatabaseConnection, query: PageRequest) -> Result<PageResponse<sys_role_btns::Model>, ServiceError> {
    ...
    pub async fn get_by_composite_id(db: &DatabaseConnection, role_id: u64, sys_menu_id: u64, sys_base_menu_btn_id: u64) -> Result<sys_role_btns::Model, ServiceError> {
        SysRoleBtns::find()
            ...
            .one(db)
            .await?
            .ok_or_else(|| ServiceError::NotFound("角色按钮关联不存在".into()))
    }

    pub async fn delete(db: &DatabaseConnection, role_id: u64, sys_menu_id: u64, sys_base_menu_btn_id: u64) -> Result<(), ServiceError> {
